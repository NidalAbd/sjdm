...
if (!function_exists('platformUrl')) {
    /**
     * Generate platform URL for current language
     *
     * @param string $platform
     * @param string|null $locale
     * @return string
     */
    function platformUrl($platform, $locale = null)
    {
        $locale = $locale ?: app()->getLocale();

        if ($locale === 'en') {
            return route('platform.show', $platform);
        } else {
            return route('platform.show.localized', ['locale' => $locale, 'platform' => $platform]);
        }
    }
}
...
if (!function_exists('categoryUrl')) {
    /**
     * Generate category URL for current language
     *
     * @param string $category
     * @param string|null $locale
     * @return string
     */
    function categoryUrl($category, $locale = null)
    {
        $locale = $locale ?: app()->getLocale();
        ...
        if ($locale === 'en') {
            ...
            return route('category.show.localized', ['locale' => $locale, 'category' => $category]);
        }
    }
}
